...
pub const Machine = struct {
    ...
    pub fn from_file(allocator: std.mem.Allocator, file_path: []const u8) !Machine {
        var res: Machine = .{
            .allocator = allocator,
    ...
    pub fn display(self: *Machine) !void {
    ...
    pub fn step(self: *Machine) !bool {
        ...
        switch (try u8_to_inst(self.program.items[self.pc])) {
            ...
            .halt => {
                try wemVM.info("Machine halted successfully!\n", .{});
                return false;
            },
            .mov => {
                self.pc += 1;
